...
contains one word. All the words consist of lowercase Latin letters and possess the lengths of from 1 to 100 characters.
...
int main()
{
    ...
    for (int idx = 0; idx != number+1; ++idx)
    {
        ...
        if (lines[idx].size() > 10)
        {
            cout<<lines[idx].front()<<lines[idx].size()-2<<lines[idx].back()<<endl;
        }
        else
        {
            cout<<lines[idx]<<endl;
    ...
    //     if (lines[idx].size() >> 10)
    //     {
    //         cout<<lines[idx].front()<<lines[idx].length-2<<lines[idx].back()<<endl;
    //     }
    // }
}