...
Question: There is a game called "I Wanna Be the Guy", consisting of n levels. 
...
These integers denote the indices of levels Little X can pass. The next line contains the levels Little Y 
...
int main()
{
    ...
    set<int> lvls;
    for (int i = 1; i <= n; i++) lvls.insert(i);

    int p;
    cin >> p;
    set<int> xy;
    for (int i = 0, x; i < p && cin >> x; i++) xy.insert(x);
    
    int q;
    cin >> q;
    for (int i = 0, x; i < q && cin >> x; i++) xy.insert(x);

    if (lvls.size() == xy.size()) cout << "I become the guy." << endl;
