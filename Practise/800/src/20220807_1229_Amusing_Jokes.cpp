...
Question: So, the New Year holidays are over. Santa Claus and his colleagues 
can take a rest and have guests at last. When two "New Year and Christmas Men" meet, 
thear assistants cut out of cardboard the letters from the guest's name and the host's 
...
But everybody wondered whether it is possible to restore the names of the host and 
his guests from the letters lying at the door? That is, we need to verify that there 
...
the second line contains the name of the residence host and the third line contains 
letters in a pile that were found at the door in the morning. All lines are not empty 
and contain only uppercase Latin letters. The length of each line does not exceed 100.
...
int main()
{
	string line1;	cin >> line1;
	string line2;	cin >> line2;
	string line3;	cin >> line3;

	// combine line1 and line2
	string combine = line1+line2;
	
	sort(combine.begin(), combine.end());
	sort(line3.begin(), line3.end());

	if (combine == line3) cout << "YES\n";
	else cout << "NO\n";
	return 0;
}
